 ...
 *
 * It began life inside the how-to-play sheet and is shared now, because the
 * index wants the same picture at a different size: a game explains itself far
 * better with a solved board beside it than with another paragraph. The class
 * is a prop rather than a second copy of four drawings -- one drawing, two
 * places it is sized.
 */
export default function Diagram({ id, className = 'diagram' }: { id: string; className?: string }) {
  const U = 40;
  if (id === 'zip') {
    ...
    return (
      <svg className={className} viewBox="-6 -6 132 132" role="img"
