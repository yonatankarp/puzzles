/// <reference lib="webworker" />
import { generateDifficulty, type DifficultyName, type QueensPuzzle } from './engine.ts';
import { generateDaily } from './daily.ts';

export interface QueensRequest {
  id: number;
  kind: 'daily' | 'tier';
  tier?: DifficultyName;
  seed?: number;
}

export interface QueensReply {
  id: number;
  puzzle: QueensPuzzle | null;
}

self.addEventListener('message', (event: MessageEvent<QueensRequest>) => {
  const { id, kind, tier, seed } = event.data;
  const puzzle = kind === 'daily' ? generateDaily(seed!) : generateDifficulty(tier!, seed);
  (self as unknown as Worker).postMessage({ id, puzzle } satisfies QueensReply);
});
