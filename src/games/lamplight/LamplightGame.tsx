...
declare global {
  interface Window { __lamplight?: ReturnType<LampCore['testHooks']> }
}

export default function LamplightGame({ shared, seed, onBack }: Props) {
  const coreRef = useRef<LampCore | null>(null);
  coreRef.current ??= new LampCore();
  ...
  useEffect(() => {
    ...
    if (import.meta.env.MODE !== 'production' && location.search.includes('test')) {
      window.__lamplight = core.testHooks();
    }
    return () => {
      unsubscribe();
      core.destroy();
      if (import.meta.env.MODE !== 'production') delete window.__lamplight;
    };
  }, [core]);
  ...
  return (
    <>
      ...
      <div className="wrap">
        ...
        <div className="hint">
          drag out of a lamp to aim it · tap to turn it round
          <span className="keys">
            {' · '}<kbd>space</kbd> next lamp ·{' '}
            <kbd>←</kbd><kbd>↑</kbd><kbd>↓</kbd><kbd>→</kbd> shine ·{' '}
            <kbd>enter</kbd> turn · <kbd>U</kbd> undo · <kbd>H</kbd> hint ·{' '}
            <kbd>R</kbd> restart · <kbd>N</kbd> new
          </span>
        </div>
      </div>
    </>
